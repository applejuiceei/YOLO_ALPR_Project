        ...
        while True:
            ...
            if detect_vehicle_now:
                ...
                if detect_roi is not None:
                    ...
                    detect_offset = (rx1, ry1)
                    run_stats["vehicle_detection_roi_frames"] += 1
                vehicle_rgb = cv2.cvtColor(cv2.resize(detect_frame, (640, 640)), cv2.COLOR_BGR2RGB)
                boxes, confidences = decode_vehicle(vehicle_model.infer_rgb(vehicle_rgb), detect_frame.shape, args.vehicle_conf, 0.45)
                boxes = offset_boxes(boxes, detect_offset[0], detect_offset[1], frame.shape)
