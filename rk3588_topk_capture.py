...
def vote(
    ...
    state.vote_events.append({"frame_idx": frame_idx, "text": text, "confidence": confidence, "weight": weight})
...
def save_results(states: dict[int, TrackState], output_dir: Path, run_stats: dict[str, Any]) -> None:
    run_summary: dict[str, Any] = {"stats": run_stats, "tracks": {}, "ignored_tracks": {}}
    for track_id, state in sorted(states.items()):
        ...
        for rank, candidate in enumerate(candidates, 1):
            ...
            record.pop("full_frame", None)
            record.pop("vehicle_crop", None)
            record.pop("plate_crop", None)
            record["rank"] = rank
...
def main() -> None:
    ...
    parser.add_argument("--ocr-engine", choices=("hyperlpr3", "rknn"), default="hyperlpr3")
    ...
    try:
        while True:
                ...
                if ready:
                    ...
                    if roi.size:
                        ...
                        for corners, obb_score in decode_obbs(plate_model.infer_rgb(plate_rgb), roi, (rx1, ry1), args.plate_imgsz, args.plate_conf):
                            ...
                            accepted, rejection, plate_area, _info = filter_plate(corners, obb_score, box, args)
                            if not accepted:
                                state.rejected[rejection] += 1
            ...
            if processed_frames % args.progress_interval == 0:
                print(f"Processed {processed_frames} frames (video frame {frame_idx}) | tracks={len(states)} | plate_hits={plate_hits} | fps={processed_frames / elapsed:.2f}", flush=True)
        ...
        save_results(states, output_dir, dict(run_stats))
        print(f"Run stats: {dict(run_stats)}")
    print(f"Saved Top-K capture run: {output_dir}")
