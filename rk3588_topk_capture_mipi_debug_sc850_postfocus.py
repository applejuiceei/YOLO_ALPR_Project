...
class TrackState:
    ...
    mipi_plate_change_text: str | None = None
    mipi_plate_change_count: int = 0
...
class V4L2CtlCapture:
    ...
    def __init__(self, device: str, width: int, height: int, fourcc: str, color_mode: str, raw10_stride: int = 0) -> None:
        self.width = width
        self.height = height
        self.fourcc = fourcc
        self.color_mode = color_mode
        self.raw10_stride = raw10_stride if raw10_stride > 0 else width * 5 // 4
        if color_mode == "uyvy":
            self.frame_size = width * height * 2
        elif color_mode == "raw10-gray":
            self.frame_size = self.raw10_stride * height
        else:
    ...
    def read(self) -> tuple[bool, np.ndarray | None]:
        ...
        return True, decode_mipi_frame(raw, self.width, self.height, self.color_mode, self.raw10_stride)
...
class LatestFramePublisher:
    """Encode only the newest pending preview frame on a worker thread."""

    def __init__(self, path: Path, max_width: int, jpeg_quality: int) -> None:
        self.path = path
    ...
    def submit(self, frame: np.ndarray) -> None:
        with self.condition:
            ...
            if self.pending is not None:
                self.dropped += 1
            # The main loop does not mutate this annotated frame after submit.
            # Keeping the ndarray reference avoids another 4K frame copy.
            self.pending = frame
    ...
    def close(self, timeout: float = 10.0) -> None:
        ...
        if self.thread.is_alive():
            self.errors += 1
            self.last_error = f"preview worker did not stop within {timeout:.1f}s"
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                temporary = self.path.with_name(f"{self.path.stem}_tmp{self.path.suffix}")
                preview = frame
                if self.max_width and frame.shape[1] > self.max_width:
                    preview_height = max(1, int(round(frame.shape[0] * self.max_width / frame.shape[1])))
                    preview = cv2.resize(frame, (self.max_width, preview_height), interpolation=cv2.INTER_AREA)
                ok = cv2.imwrite(
                    str(temporary),
                    preview,
...
def artifact_full_frame(frame: np.ndarray, max_width: int) -> np.ndarray:
    """Create a bounded full-frame preview for retained Top-K diagnostics."""
...
def decode_mipi_frame(raw: np.ndarray, width: int, height: int, mode: str, raw10_stride: int = 0) -> np.ndarray:
    """Turn V4L2 camera data into BGR, including SC850SL RAW10 grayscale."""
    if mode == "opencv":
        if raw.ndim != 3 or raw.shape[2] != 3:
            raise ValueError(f"OpenCV MIPI conversion returned unexpected shape: {raw.shape}")
        return raw
    if mode == "raw10-gray":
        stride = raw10_stride if raw10_stride > 0 else width * 5 // 4
        expected_size = stride * height
        if raw.size != expected_size:
            raise ValueError(
                f"MIPI RAW10 buffer size mismatch: got {raw.size}, expected {expected_size} for {width}x{height} stride={stride}."
            )
        packed = raw.reshape(height, stride)[:, : width * 5 // 4]
        blocks = packed.reshape(height, -1, 5).astype(np.uint16)
        img10 = np.empty((height, width), dtype=np.uint16)
        img10[:, 0::4] = (blocks[:, :, 0] << 2) | (blocks[:, :, 4] & 0x03)
        img10[:, 1::4] = (blocks[:, :, 1] << 2) | ((blocks[:, :, 4] >> 2) & 0x03)
        img10[:, 2::4] = (blocks[:, :, 2] << 2) | ((blocks[:, :, 4] >> 4) & 0x03)
        img10[:, 3::4] = (blocks[:, :, 3] << 2) | ((blocks[:, :, 4] >> 6) & 0x03)
        sample = img10[::8, ::8]
        lo, hi = np.percentile(sample, [1, 99])
        gray = np.clip((img10.astype(np.float32) - lo) * 255.0 / max(1.0, hi - lo), 0, 255).astype(np.uint8)
...
def gray_world_white_balance(frame: np.ndarray) -> np.ndarray:
    ...
    mask = (gray > 20) & (gray < 220) & (channel_max < 245) & ((channel_max - channel_min) < 90)
    if int(mask.sum()) < 1000:
        mask = (gray > 20) & (gray < 235) & (channel_max < 250)
    pixels = frame[mask] if int(mask.sum()) >= 1000 else frame.reshape(-1, 3)
    means = pixels.reshape(-1, 3).mean(axis=0)
    target = float(means.mean())
    gains = np.clip(target / np.maximum(means, 1.0), 0.65, 1.55)
    return np.clip(frame.astype(np.float32) * gains, 0, 255).astype(np.uint8)


def low_light_enhance(frame: np.ndarray, gamma: float, clahe_clip: float) -> np.ndarray:
    gamma = max(0.25, min(1.0, float(gamma)))
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)
    if clahe_clip > 0:
        clahe = cv2.createCLAHE(clipLimit=float(clahe_clip), tileGridSize=(8, 8))
        v = clahe.apply(v)
    lookup = np.array([((value / 255.0) ** gamma) * 255.0 for value in range(256)], dtype=np.uint8)
    v = cv2.LUT(v, lookup)
...
def is_strict_chinese_plate_text(text: str) -> bool:
    if len(text) not in (7, 8):
        return False
    if text[0] not in CHINESE_PLATE_PROVINCES:
        return False
    if not ("A" <= text[1] <= "Z"):
        return False
    return all(char.isdigit() or ("A" <= char <= "Z") for char in text[2:])
...
def resolve_mipi_plate_scan_roi(args: argparse.Namespace, frame_shape: tuple[int, int, int]) -> list[int]:
    values = getattr(args, "mipi_plate_scan_roi", None)
    if values:
        frame_h, frame_w = frame_shape[:2]
...
def mipi_plate_scan_fallback(
    ...
    label_renderer: PlateLabelRenderer,
) -> int:
    if args.ocr_engine != "hyperlpr3" or hyperlpr3 is None:
        return 0
    roi_box = resolve_mipi_plate_scan_roi(args, raw_frame.shape)
    rx1, ry1, rx2, ry2 = roi_box
    roi = raw_frame[ry1:ry2, rx1:rx2]
    if not roi.size:
        return 0

    scan_track_id = args.mipi_plate_scan_track_id
    state = states.setdefault(scan_track_id, TrackState())
    state.seen_frames += 1
    run_stats["mipi_plate_scan_attempts"] += 1
    ...
    hits = 0
    for corners, obb_score in decode_obbs(plate_model.infer_rgb(plate_rgb), roi, (rx1, ry1), args.plate_imgsz, args.mipi_plate_scan_conf):
        run_stats["mipi_plate_scan_candidates"] += 1
        plate = warp_plate(raw_frame, corners)
        text, ocr_conf = recognize_hyperlpr3(hyperlpr3, plate)
        if (
            text
            and state.locked_text
            and text != state.locked_text
            and ocr_conf >= args.mipi_plate_scan_reset_conf
        ):
            if state.mipi_plate_change_text == text:
                state.mipi_plate_change_count += 1
            else:
                state.mipi_plate_change_text = text
                state.mipi_plate_change_count = 1
            if state.mipi_plate_change_count >= args.mipi_plate_scan_reset_votes:
                old_text = state.locked_text
                state = TrackState()
                states[scan_track_id] = state
                run_stats["mipi_plate_scan_resets"] += 1
                print(f"[MIPI-SCAN RESET] track={scan_track_id} {old_text} -> {text} frame={frame_idx}", flush=True)
        elif text == state.locked_text:
            state.mipi_plate_change_text = None
            state.mipi_plate_change_count = 0
        plate_area = polygon_area(corners)
        ...
        candidate = Candidate(
            ...
            roi_box,
            1.0,
            artifact_full_frame(raw_frame, args.artifact_full_frame_max_width),
            roi.copy(),
            plate,
            text or None,
            float(ocr_conf),
            "hyperlpr3" if text else None,
        )
        add_candidate(state, candidate, args.topk)
        hits += 1
        run_stats["mipi_plate_scan_hits"] += 1
        if text:
            run_stats["mipi_plate_scan_ocr"] += 1
            if vote(state, text, ocr_conf, frame_idx, candidate, args, raw_frame.shape, source="plate", predicted=False, run_stats=run_stats):
                print(f"[LOCK] track={scan_track_id} plate={state.locked_text} frame={frame_idx} source=mipi_plate_scan", flush=True)
        cv2.polylines(frame, [corners.astype(np.int32)], True, (0, 255, 255), 2)
        label = text or state.locked_text or f"plate {float(obb_score):.2f}"
        origin = (int(corners[:, 0].min()), max(24, int(corners[:, 1].min()) - 8))
        label_renderer.draw(frame, label, origin, (0, 255, 255))
        break
    return hits
...
def resolve_process_roi(args: argparse.Namespace, frame_shape: tuple[int, int, int]) -> list[int] | None:
    ...
    return resolve_roi_values(values, frame_shape)


def resolve_detect_roi(args: argparse.Namespace, frame_shape: tuple[int, int, int]) -> list[int] | None:
    values = getattr(args, "detect_roi", None)
    return resolve_roi_values(values, frame_shape)


def resolve_roi_values(values: list[float] | None, frame_shape: tuple[int, int, int]) -> list[int] | None:
    if not values:
        return None
    frame_h, frame_w = frame_shape[:2]
    x1, y1, x2, y2 = [float(value) for value in values]
...
def draw_detect_roi(frame: np.ndarray, roi: list[int] | None) -> None:
    if roi is None:
        return
    x1, y1, x2, y2 = roi
    cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 255), 2)
    cv2.putText(frame, "DETECT ROI", (x1, min(frame.shape[0] - 8, y2 + 24)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 0, 255), 2, cv2.LINE_AA)


def offset_boxes(boxes: np.ndarray, offset_x: int, offset_y: int, frame_shape: tuple[int, int, int]) -> np.ndarray:
    if boxes.size == 0:
        return boxes
    shifted = boxes.copy()
    shifted[:, [0, 2]] += float(offset_x)
    shifted[:, [1, 3]] += float(offset_y)
    frame_h, frame_w = frame_shape[:2]
    shifted[:, [0, 2]] = np.clip(shifted[:, [0, 2]], 0, frame_w)
    shifted[:, [1, 3]] = np.clip(shifted[:, [1, 3]], 0, frame_h)
    return shifted


def nms_xyxy(boxes: np.ndarray, confidences: np.ndarray, confidence: float, nms_iou: float) -> tuple[np.ndarray, np.ndarray]:
    if boxes.size == 0:
        return boxes, confidences
    xywh = [[float(x1), float(y1), float(x2 - x1), float(y2 - y1)] for x1, y1, x2, y2 in boxes]
    chosen = cv2.dnn.NMSBoxes(xywh, confidences.astype(float).tolist(), confidence, nms_iou)
    if len(chosen) == 0:
        return np.empty((0, 4), dtype=np.float32), np.empty((0,), dtype=np.float32)
    keep = np.asarray(chosen).reshape(-1).astype(int)
    return boxes[keep].astype(np.float32), confidences[keep].astype(np.float32)


def split_detect_regions(roi: list[int] | None, frame_shape: tuple[int, int, int], rows: int, cols: int, overlap: float) -> list[list[int]]:
    if roi is None:
        x1, y1, x2, y2 = 0, 0, frame_shape[1], frame_shape[0]
    else:
        x1, y1, x2, y2 = roi
    rows = max(1, int(rows))
    cols = max(1, int(cols))
    overlap = max(0.0, min(0.45, float(overlap)))
    width = max(1, x2 - x1)
    height = max(1, y2 - y1)
    base_w = width / float(cols)
    base_h = height / float(rows)
    regions: list[list[int]] = []
    for row in range(rows):
        for col in range(cols):
            tx1 = x1 + int(round(col * base_w))
            ty1 = y1 + int(round(row * base_h))
            tx2 = x1 + int(round((col + 1) * base_w))
            ty2 = y1 + int(round((row + 1) * base_h))
            pad_x = int(round((tx2 - tx1) * overlap))
            pad_y = int(round((ty2 - ty1) * overlap))
            tx1 = max(x1, tx1 - pad_x)
            ty1 = max(y1, ty1 - pad_y)
            tx2 = min(x2, tx2 + pad_x)
            ty2 = min(y2, ty2 + pad_y)
            if tx2 > tx1 and ty2 > ty1:
                regions.append([tx1, ty1, tx2, ty2])
    return regions


def motion_vehicle_boxes(
    ...
    mask = subtractor.apply(gray, learningRate=args.motion_learning_rate)
    _, mask = cv2.threshold(mask, args.motion_threshold, 255, cv2.THRESH_BINARY)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (args.motion_kernel, args.motion_kernel))
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=1)
    mask = cv2.dilate(mask, kernel, iterations=args.motion_dilate)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    boxes: list[list[float]] = []
    scores: list[float] = []
    max_area = args.motion_max_area if args.motion_max_area > 0 else region.shape[0] * region.shape[1]
    accept_roi = resolve_roi_values(args.motion_accept_roi, frame.shape) if args.motion_accept_roi else None
    for contour in contours:
        bx, by, bw, bh = cv2.boundingRect(contour)
        area = float(bw * bh)
        contour_area = float(cv2.contourArea(contour))
        aspect = bw / max(1.0, float(bh))
        full_box = [x1 + bx, y1 + by, x1 + bx + bw, y1 + by + bh]
        if bw < args.motion_min_width or bh < args.motion_min_height:
            continue
        if area < args.motion_min_area or area > max_area:
            continue
        if aspect < args.motion_min_aspect or aspect > args.motion_max_aspect:
            continue
        if args.motion_min_fill_ratio > 0 and contour_area / max(1.0, area) < args.motion_min_fill_ratio:
            continue
        if accept_roi is not None and not box_center_in_roi(full_box, accept_roi):
            continue
        boxes.append(full_box)
        scores.append(0.50 + min(0.49, area / max(1.0, float(region.shape[0] * region.shape[1])) * 5.0))
    if not boxes:
        return np.empty((0, 4), dtype=np.float32), np.empty((0,), dtype=np.float32)
...
def find_unlocked_evidence_reassociation(
    states: dict[int, TrackState],
    current_track_id: int,
    box: list[int],
    frame_idx: int,
    ...
    """Transfer evidence to a new tracker ID using its predicted vehicle box."""
    if not args.rk_adaptive or not args.unlocked_evidence_reassociate:
        return None
    candidate_box = np.asarray(box, dtype=np.float32)
    best_track_id: int | None = None
    best_score = 0.0
    ...
    for track_id, state in states.items():
        if (
            ...
            or state.locked_text
            or state.last_box is None
            or state.last_detection_frame is None
            or state.seen_frames < args.unlocked_reassoc_min_seen
            or len(state.vote_events) < args.unlocked_reassoc_min_votes
        ):
            continue
        missing = frame_idx - state.last_detection_frame
        if not 1 <= missing <= args.unlocked_reassoc_max_missing:
            continue
        reference_box = predict_box(
            state,
            frame_idx,
            frame_shape,
            args.unlocked_reassoc_max_missing,
        )
        if reference_box is None:
            continue
        reference_box = np.asarray(reference_box, dtype=np.float32)
        ...
        area_ratio = min(reference_area, candidate_area) / max(reference_area, candidate_area)
        if area_ratio < args.unlocked_reassoc_min_area_ratio:
            continue
        previous_box = np.asarray(state.last_box, dtype=np.float32)
        ...
        predicted_motion = predicted_center - previous_center
        observed_motion = candidate_center - previous_center
        predicted_norm = float(np.linalg.norm(predicted_motion))
        observed_norm = float(np.linalg.norm(observed_motion))
        if predicted_norm >= 1.0 and observed_norm >= 1.0:
            direction_cosine = float(
                np.dot(predicted_motion, observed_motion)
                / (predicted_norm * observed_norm)
            )
        else:
            direction_cosine = 1.0
        if direction_cosine < args.unlocked_reassoc_min_direction_cosine:
            continue
        if (
            iou_score < args.unlocked_reassoc_iou
            and center_score < args.unlocked_reassoc_center
        ):
            continue
        score = max(iou_score, 0.85 * center_score)
        if score > best_score:
    ...
    if best_track_id is None or best_metrics is None:
        return None
    if second_best_score > 0.0 and best_score - second_best_score < args.unlocked_reassoc_min_margin:
        return None
    best_metrics["runner_up_score"] = second_best_score
    best_metrics["margin"] = best_score - second_best_score
    return best_track_id, best_metrics
...
def prune_expired_states(states: dict[int, TrackState], frame_idx: int, args: argparse.Namespace, run_stats: Counter[str]) -> None:
    """Release finished road tracks during long-running MIPI sessions."""
    if args.state_retention_frames <= 0:
        return
    locked_retention = max(
        args.state_retention_frames,
        args.locked_reassoc_max_missing,
        args.max_locked_predict_frames,
    )
    expired: list[int] = []
    for track_id, state in states.items():
        if state.last_detection_frame is None:
            continue
        retention = locked_retention if state.locked_text else args.state_retention_frames
        if frame_idx - state.last_detection_frame > retention:
            expired.append(track_id)
    for track_id in expired:
        del states[track_id]
    if expired:
        run_stats["states_pruned"] += len(expired)
...
def vote(
    ...
    if state.locked_text or len(text) < args.min_lock_text_len or confidence < args.min_ocr_conf:
        return False
    if args.strict_plate_format and not is_strict_chinese_plate_text(text):
        if run_stats is not None:
            run_stats["ocr_rejected:plate format"] += 1
        state.lock_deferred_reason = "plate format"
        return False
...
def should_draw_track(box: list[int], confidence: float, args: argparse.Namespace) -> bool:
    x1, y1, x2, y2 = box
    width, height = max(0, x2 - x1), max(0, y2 - y1)
    return (
        confidence >= args.draw_min_vehicle_conf
        and width >= args.draw_min_vehicle_width
        and height >= args.draw_min_vehicle_height
        and width * height >= args.draw_min_vehicle_area
    )


def draw_track_if_visible(
    frame: np.ndarray,
    box: list[int],
    track_id: int,
    state: TrackState,
    ...
    if should_draw_track(box, confidence, args):
        draw_track(frame, box, track_id, state, renderer, predicted=predicted)
...
def save_lock_evidence(
    ...
    """Persist the OCR evidence before short-lived live state can be pruned."""
    if not state.locked_text or state.locked_frame is None:
        return
    try:
        lock_dir = output_dir / "locks" / f"lock_{state.locked_frame:06d}_track_{track_id}"
        lock_dir.mkdir(parents=True, exist_ok=True)
        consensus = state.locked_consensus or {}
        vote_frames = {int(frame) for frame in consensus.get("distinct_frames", [])}
        evidence_by_frame: dict[int, Candidate] = {}
        for candidate in state.candidates + [lock_candidate]:
            if candidate.ocr_text != state.locked_text:
                continue
            if vote_frames and candidate.frame_idx not in vote_frames:
                continue
            evidence_by_frame[candidate.frame_idx] = candidate
        evidence = [evidence_by_frame[frame] for frame in sorted(evidence_by_frame)]
        if not evidence:
            evidence = [lock_candidate]

        saved_candidates: list[dict[str, Any]] = []
        for rank, candidate in enumerate(evidence, 1):
            cv2.imwrite(str(lock_dir / f"full_vote{rank}.jpg"), candidate.full_frame)
            cv2.imwrite(str(lock_dir / f"vehicle_vote{rank}.jpg"), candidate.vehicle_crop)
            cv2.imwrite(str(lock_dir / f"plate_vote{rank}.jpg"), candidate.plate_crop)
            saved_candidates.append(candidate_summary(candidate, rank))

        evidence_summary = {
            "track_id": track_id,
            "locked_text": state.locked_text,
            "locked_frame": state.locked_frame,
            "locked_consensus": consensus,
            "vote_events": [
                event for event in state.vote_events if event.get("text") == state.locked_text
            ],
        ...
        run_stats["lock_evidence_saved"] += 1
        print(f"[LOCK EVIDENCE] {lock_dir}", flush=True)
    except Exception as exc:
        run_stats["lock_evidence_errors"] += 1
        print(f"[LOCK EVIDENCE ERROR] track={track_id} frame={state.locked_frame}: {exc}", flush=True)


def emit_lock_event(
    recent_lock_texts: dict[str, int],
    text: str,
    frame_idx: int,
    args: argparse.Namespace,
    run_stats: Counter[str],
) -> bool:
    """Suppress repeated lock events for the same plate without reintroducing vehicle tracking."""
    window = args.lock_text_dedupe_frames
    if window <= 0:
        run_stats["lock_events_emitted"] += 1
        return True
    stale_texts = [
        locked_text
        for locked_text, last_frame in recent_lock_texts.items()
        if frame_idx - last_frame > window
    ]
    for locked_text in stale_texts:
        del recent_lock_texts[locked_text]
    previous_frame = recent_lock_texts.get(text)
    recent_lock_texts[text] = frame_idx
    if previous_frame is not None and frame_idx - previous_frame <= window:
        run_stats["lock_events_deduplicated"] += 1
        return False
    run_stats["lock_events_emitted"] += 1
    return True
...
def main() -> None:
    ...
    parser.add_argument("--camera-device", default="/dev/video53", help="V4L2 device used when --video mipi (default: 4K MIPI main path).")
    ...
    parser.add_argument("--mipi-fourcc", choices=("UYVY", "NV12", "NV21", "BG10"), default="UYVY", help="Requested MIPI output pixel format. BG10 is for SC850SL RAW10 grayscale probing.")
    parser.add_argument("--mipi-color-mode", choices=("uyvy", "nv12", "nv21", "opencv", "raw10-gray"), default="uyvy", help="MIPI pixel decoding. Keep this aligned with --mipi-fourcc unless using OpenCV conversion.")
    ...
    parser.add_argument("--mipi-low-light", action="store_true", help="Brighten dark MIPI frames with HSV gamma/CLAHE for phone-screen or backlit tests.")
    parser.add_argument("--mipi-gamma", type=float, default=0.65, help="Gamma used by --mipi-low-light. Lower values brighten shadows more.")
    parser.add_argument("--mipi-clahe-clip", type=float, default=1.5, help="CLAHE clip limit used by --mipi-low-light. Use 0 to disable CLAHE.")
    ...
    parser.add_argument("--raw10-stride", type=int, default=0, help="Bytes per RAW10 line for --mipi-color-mode raw10-gray; SC850SL 3840x2160 BG10 currently reports 4864.")
    ...
    parser.add_argument(
        "--artifact-full-frame-max-width",
        type=int,
        default=960,
        help="Maximum width for retained full-frame Top-K/rejected previews; use 0 to retain native resolution.",
    )
    parser.add_argument("--vehicle-conf", type=float, default=0.55)
    parser.add_argument("--vehicle-source", choices=("rknn", "motion", "hybrid"), default="rknn", help="Vehicle proposal source. motion is a fixed-camera fallback for SC850SL grayscale road tests; hybrid combines motion with RKNN.")
    ...
    parser.add_argument(
        ...
        nargs=4,
        metavar=("X1", "Y1", "X2", "Y2"),
        help="Crop this ROI before vehicle detection, then map detections back to full-frame coordinates. Values <=1 are normalized.",
    )
    parser.add_argument("--draw-detect-roi", action="store_true", help="Draw --detect-roi on published/saved frames.")
    parser.add_argument("--detect-roi-tiles", type=int, nargs=2, metavar=("ROWS", "COLS"), default=(1, 1), help="Split --detect-roi into ROWS x COLS vehicle-detection tiles before resizing to 640. Useful for distant small vehicles.")
    parser.add_argument("--detect-roi-overlap", type=float, default=0.15, help="Normalized overlap added around each --detect-roi tile, 0.0-0.45.")
    ...
    parser.add_argument("--draw-min-vehicle-conf", type=float, default=0.0, help="Minimum vehicle confidence required to draw a track box.")
    parser.add_argument("--draw-min-vehicle-width", type=int, default=0, help="Minimum vehicle width in pixels required to draw a track box.")
    parser.add_argument("--draw-min-vehicle-height", type=int, default=0, help="Minimum vehicle height in pixels required to draw a track box.")
    parser.add_argument("--draw-min-vehicle-area", type=int, default=0, help="Minimum vehicle area in pixels required to draw a track box.")
    ...
    parser.add_argument("--motion-history", type=int, default=120, help="Background history for --vehicle-source motion/hybrid.")
    parser.add_argument("--motion-var-threshold", type=float, default=24.0, help="OpenCV MOG2 variance threshold for motion proposals.")
    parser.add_argument("--motion-learning-rate", type=float, default=-1.0, help="Background learning rate; -1 lets OpenCV choose.")
    parser.add_argument("--motion-threshold", type=int, default=180, help="Foreground mask threshold for motion proposals.")
    parser.add_argument("--motion-kernel", type=int, default=5, help="Morphology kernel size for motion proposals.")
    parser.add_argument("--motion-dilate", type=int, default=2, help="Dilation iterations for motion proposals.")
    parser.add_argument("--motion-min-width", type=int, default=35, help="Minimum motion box width in full-resolution pixels.")
    parser.add_argument("--motion-min-height", type=int, default=25, help="Minimum motion box height in full-resolution pixels.")
    parser.add_argument("--motion-min-area", type=int, default=1800, help="Minimum motion box area in full-resolution pixels.")
    parser.add_argument("--motion-max-area", type=int, default=0, help="Maximum motion box area; 0 disables the upper bound.")
    parser.add_argument("--motion-min-aspect", type=float, default=0.0, help="Minimum width/height ratio for motion proposals; use this to reject tall skinny e-bike/pedestrian/wheel fragments.")
    parser.add_argument("--motion-max-aspect", type=float, default=8.0, help="Maximum width/height ratio for motion proposals.")
    parser.add_argument("--motion-min-fill-ratio", type=float, default=0.0, help="Minimum contour-area / bounding-box-area ratio for motion proposals; use this to reject sparse lane-line/noise boxes.")
    parser.add_argument("--motion-accept-roi", nargs=4, type=float, metavar=("X1", "Y1", "X2", "Y2"), help="Only keep motion boxes whose center is inside this ROI. Values <=1 are treated as normalized coordinates.")
    ...
    parser.add_argument("--unlocked-evidence-reassociate", action=argparse.BooleanOptionalAction, default=False, help="Transfer an unlocked state with OCR votes to a new tracker ID using its Kalman-predicted vehicle box.")
    parser.add_argument("--unlocked-reassoc-max-missing", type=int, default=4, help="Maximum missing processed frames for unlocked evidence reassociation.")
    parser.add_argument("--unlocked-reassoc-min-seen", type=int, default=2, help="Minimum observed frames required before an unlocked state can be reassociated.")
    parser.add_argument("--unlocked-reassoc-min-votes", type=int, default=1, help="Minimum OCR vote events required before an unlocked state can be reassociated.")
    parser.add_argument("--unlocked-reassoc-iou", type=float, default=0.05, help="Minimum predicted-box IoU for unlocked evidence reassociation.")
    parser.add_argument("--unlocked-reassoc-center", type=float, default=0.28, help="Fallback predicted center/scale affinity for unlocked evidence reassociation.")
    parser.add_argument("--unlocked-reassoc-min-area-ratio", type=float, default=0.60, help="Minimum predicted/current vehicle-box area ratio for unlocked evidence reassociation.")
    parser.add_argument("--unlocked-reassoc-min-direction-cosine", type=float, default=0.80, help="Minimum agreement between predicted and observed vehicle motion directions.")
    parser.add_argument("--unlocked-reassoc-min-margin", type=float, default=0.10, help="Minimum best-versus-runner-up geometry score margin for unlocked evidence reassociation.")
    ...
    parser.add_argument("--lock-text-dedupe-frames", type=int, default=0, help="Suppress repeated lock events for the same plate text within this many processed frames; 0 disables event deduplication.")
    ...
    parser.add_argument("--strict-plate-format", action=argparse.BooleanOptionalAction, default=False, help="Require Chinese plate text to look like province + letter + alphanumeric serial before it can enter voting.")
    ...
    parser.add_argument(
        "--state-retention-frames",
        type=int,
        default=0,
        help="Forget inactive track state after this many processed frames; 0 keeps all states for bounded diagnostic runs.",
    )
    ...
    parser.add_argument("--publish-async", action=argparse.BooleanOptionalAction, default=False, help="Encode only the latest pending preview frame on a background thread so JPEG publication does not block inference.")
    parser.add_argument("--preview-jpeg-quality", type=int, default=80, help="JPEG quality for --publish-frame, 1-100.")
    parser.add_argument("--preview-max-width", type=int, default=0, help="Maximum width of published preview JPEG; 0 keeps the processed frame width.")
    parser.add_argument("--mipi-plate-scan-fallback", action="store_true", help="MIPI debug mode: when normal vehicle-gated flow has no new plate hit, scan a frame ROI directly for plates.")
    parser.add_argument("--mipi-plate-scan-only", action="store_true", help="MIPI debug mode: skip vehicle detection/tracking and publish only direct plate-scan results.")
    parser.add_argument("--mipi-plate-scan-interval", type=int, default=1, help="Run --mipi-plate-scan-fallback every N processed frames.")
    parser.add_argument("--mipi-plate-scan-conf", type=float, default=0.20, help="OBB confidence threshold for --mipi-plate-scan-fallback.")
    parser.add_argument("--mipi-plate-scan-roi", nargs=4, type=float, metavar=("X1", "Y1", "X2", "Y2"), help="ROI for --mipi-plate-scan-fallback. Values <=1 are treated as normalized coordinates.")
    parser.add_argument("--mipi-plate-scan-track-id", type=int, default=900001, help="Synthetic track id used by --mipi-plate-scan-fallback voting.")
    parser.add_argument("--mipi-plate-scan-reset-conf", type=float, default=0.85, help="Reset the synthetic MIPI plate-scan track when a different high-confidence plate appears.")
    parser.add_argument("--mipi-plate-scan-reset-votes", type=int, default=2, help="Consecutive different OCR reads required before resetting the synthetic MIPI plate-scan track.")
    args = parser.parse_args()
    if args.vehicle_detect_interval <= 0:
        raise ValueError("--vehicle-detect-interval must be positive")
    if args.motion_history <= 0 or args.motion_kernel <= 0 or args.motion_dilate < 0:
        raise ValueError("motion history/kernel/dilate values are invalid")
    if args.motion_min_width <= 0 or args.motion_min_height <= 0 or args.motion_min_area <= 0 or args.motion_max_aspect <= 0:
        raise ValueError("motion size/aspect filters must be positive")
    if args.motion_min_aspect < 0 or args.motion_min_aspect > args.motion_max_aspect:
        raise ValueError("--motion-min-aspect must be >= 0 and <= --motion-max-aspect")
    if not 0 <= args.motion_min_fill_ratio <= 1:
        raise ValueError("--motion-min-fill-ratio must be between 0 and 1")
    if args.motion_accept_roi and (args.motion_accept_roi[2] <= args.motion_accept_roi[0] or args.motion_accept_roi[3] <= args.motion_accept_roi[1]):
        raise ValueError("--motion-accept-roi must be X1 Y1 X2 Y2 with X2 > X1 and Y2 > Y1")
    ...
    if args.adaptive_min_exact_votes <= 0 or args.adaptive_min_vote_frames <= 0 or args.adaptive_min_strong_votes < 0:
        raise ValueError("adaptive exact/frame vote thresholds must be positive and strong votes must be non-negative")
    if args.locked_reassoc_max_missing < 0:
        raise ValueError("--locked-reassoc-max-missing must be non-negative")
    if (
        args.unlocked_reassoc_max_missing < 0
        or args.unlocked_reassoc_min_seen <= 0
        or args.unlocked_reassoc_min_votes <= 0
    ):
        raise ValueError("unlocked reassociation frame/vote limits are invalid")
    if (
        not 0.0 <= args.unlocked_reassoc_iou <= 1.0
        or not 0.0 <= args.unlocked_reassoc_center <= 1.0
        or not 0.0 <= args.unlocked_reassoc_min_area_ratio <= 1.0
        or not -1.0 <= args.unlocked_reassoc_min_direction_cosine <= 1.0
        or not 0.0 <= args.unlocked_reassoc_min_margin <= 1.0
    ):
        raise ValueError("unlocked reassociation thresholds must be between 0 and 1")
    if args.plate_attempt_interval <= 0:
        raise ValueError("--plate-attempt-interval must be positive")
    if args.lock_text_dedupe_frames < 0:
        raise ValueError("--lock-text-dedupe-frames must be non-negative")
    if args.max_rejected_per_track < 0:
        raise ValueError("--max-rejected-per-track must be non-negative")
    if args.artifact_full_frame_max_width < 0:
        raise ValueError("--artifact-full-frame-max-width must be non-negative")
    ...
    if args.state_retention_frames < 0:
        raise ValueError("--state-retention-frames must be non-negative")
    if not 0.0 <= args.draw_min_vehicle_conf <= 1.0:
        raise ValueError("--draw-min-vehicle-conf must be between 0 and 1")
    if args.draw_min_vehicle_width < 0 or args.draw_min_vehicle_height < 0 or args.draw_min_vehicle_area < 0:
        raise ValueError("minimum draw vehicle dimensions/area must be non-negative")
    ...
    if args.detect_roi and (args.detect_roi[2] <= args.detect_roi[0] or args.detect_roi[3] <= args.detect_roi[1]):
        raise ValueError("--detect-roi must be X1 Y1 X2 Y2 with X2 > X1 and Y2 > Y1")
    if args.detect_roi_tiles[0] <= 0 or args.detect_roi_tiles[1] <= 0:
        raise ValueError("--detect-roi-tiles values must be positive")
    if not 0.0 <= args.detect_roi_overlap <= 0.45:
        raise ValueError("--detect-roi-overlap must be between 0.0 and 0.45")
    ...
    if args.preview_max_width < 0:
        raise ValueError("--preview-max-width must be non-negative")
    if args.publish_async and args.publish_frame is None:
        raise ValueError("--publish-async requires --publish-frame")
    if args.mipi_plate_scan_interval <= 0:
        raise ValueError("--mipi-plate-scan-interval must be positive")
    if args.mipi_plate_scan_reset_votes <= 0:
        raise ValueError("--mipi-plate-scan-reset-votes must be positive")
    if args.mipi_plate_scan_roi and (args.mipi_plate_scan_roi[2] <= args.mipi_plate_scan_roi[0] or args.mipi_plate_scan_roi[3] <= args.mipi_plate_scan_roi[1]):
        raise ValueError("--mipi-plate-scan-roi must be X1 Y1 X2 Y2 with X2 > X1 and Y2 > Y1")
    if args.mipi_gamma <= 0:
        raise ValueError("--mipi-gamma must be positive")
    if args.mipi_clahe_clip < 0:
        raise ValueError("--mipi-clahe-clip must be non-negative")
    ...
    if args.video == "mipi":
        mode_by_fourcc = {"UYVY": "uyvy", "NV12": "nv12", "NV21": "nv21", "BG10": "raw10-gray"}
        ...
        if args.mipi_backend == "v4l2ctl":
            cap = V4L2CtlCapture(args.camera_device, args.camera_width, args.camera_height, args.mipi_fourcc, args.mipi_color_mode, args.raw10_stride)
        else:
    ...
    preview_publisher: LatestFramePublisher | None = None
    motion_subtractor = (
        ...
        if args.vehicle_source in {"motion", "hybrid"}
        else None
    )
    try:
        if args.publish_async and args.publish_frame is not None:
            preview_publisher = LatestFramePublisher(
                args.publish_frame,
        ...
        while True:
            ...
            if args.video == "mipi" and args.mipi_backend == "opencv":
                frame = decode_mipi_frame(captured, mipi_width, mipi_height, args.mipi_color_mode, args.raw10_stride)
            else:
                frame = captured
            if args.video == "mipi" and args.mipi_gray_world:
                frame = gray_world_white_balance(frame)
            if args.video == "mipi" and args.mipi_low_light:
                frame = low_light_enhance(frame, args.mipi_gamma, args.mipi_clahe_clip)
            frame_idx += 1
            processed_frames += 1
            raw_frame = frame.copy()
            artifact_frame = artifact_full_frame(raw_frame, args.artifact_full_frame_max_width)
            process_roi = resolve_process_roi(args, frame.shape)
            detect_roi = resolve_detect_roi(args, frame.shape)
            if args.draw_process_roi:
                draw_process_roi(frame, process_roi)
            if args.draw_detect_roi:
                draw_detect_roi(frame, detect_roi)
            ...
            vehicle_box_count = 0
            if args.video == "mipi" and args.mipi_plate_scan_only:
                run_stats["vehicle_skipped:mipi_plate_scan_only"] += 1
            elif detect_vehicle_now:
                if args.rk_adaptive and not base_detect_due:
                    run_stats["vehicle_detection_frames_adaptive"] += 1
                if detect_roi is not None:
                    run_stats["vehicle_detection_roi_frames"] += 1
                tile_boxes: list[np.ndarray] = []
                tile_confidences: list[np.ndarray] = []
                if args.vehicle_source in {"rknn", "hybrid"}:
                    rows, cols = args.detect_roi_tiles
                    regions = split_detect_regions(detect_roi, raw_frame.shape, rows, cols, args.detect_roi_overlap)
                    if len(regions) > 1:
                        run_stats["vehicle_detection_tiled_frames"] += 1
                        run_stats["vehicle_detection_tiles"] += len(regions)
                    for rx1, ry1, rx2, ry2 in regions:
                        detect_frame = raw_frame[ry1:ry2, rx1:rx2]
                        if not detect_frame.size:
                            continue
                        vehicle_rgb = cv2.cvtColor(cv2.resize(detect_frame, (640, 640)), cv2.COLOR_BGR2RGB)
                        infer_started = time.perf_counter()
                        vehicle_output = vehicle_model.infer_rgb(vehicle_rgb)
                        run_stats["time:vehicle_rknn_seconds"] += time.perf_counter() - infer_started
                        boxes_part, confidences_part = decode_vehicle(vehicle_output, detect_frame.shape, args.vehicle_conf, 0.45)
                        boxes_part = offset_boxes(boxes_part, rx1, ry1, frame.shape)
                        if boxes_part.size:
                            tile_boxes.append(boxes_part)
                            tile_confidences.append(confidences_part)
                if args.vehicle_source in {"motion", "hybrid"} and motion_subtractor is not None:
                    boxes_part, confidences_part = motion_vehicle_boxes(motion_subtractor, raw_frame, detect_roi, args)
                    run_stats["motion_vehicle_frames"] += 1
                    if boxes_part.size:
                        run_stats["motion_vehicle_boxes"] += int(len(boxes_part))
                        tile_boxes.append(boxes_part)
                        tile_confidences.append(confidences_part)
                if tile_boxes:
                    boxes = np.vstack(tile_boxes).astype(np.float32)
                    confidences = np.concatenate(tile_confidences).astype(np.float32)
                    run_stats["vehicle_detections_raw"] += int(len(boxes))
                    boxes, confidences = nms_xyxy(boxes, confidences, args.vehicle_conf, 0.45)
                else:
                    boxes = np.empty((0, 4), dtype=np.float32)
                    confidences = np.empty((0,), dtype=np.float32)
                vehicle_box_count = int(len(boxes))
                run_stats["vehicle_detections_nms"] += vehicle_box_count
            ...
            for track_id, tracked_box, vehicle_conf, is_predicted in tracked:
                ...
                if states.get(track_id) is None:
                    reassociation = find_unlocked_evidence_reassociation(
                        states,
                        track_id,
                        box,
                        frame_idx,
                    ...
                    if reassociation is not None:
                        reassociated_track_id, reassociation_metrics = reassociation
                        evidence_state = states.pop(reassociated_track_id)
                        states[track_id] = evidence_state
                        run_stats["unlocked_evidence_reassociations"] += 1
                        print(
                            f"[REASSOC VERIFYING] track {reassociated_track_id} -> {track_id} "
                            f"votes={len(evidence_state.vote_events)} frame={frame_idx} "
                            f"missing={int(reassociation_metrics['missing'])} "
                            f"iou={reassociation_metrics['iou']:.3f} "
                            f"center={reassociation_metrics['center']:.3f} "
                            f"area={reassociation_metrics['area_ratio']:.3f} "
                            f"direction={reassociation_metrics['direction_cosine']:.3f} "
                            f"margin={reassociation_metrics['margin']:.3f}",
                            flush=True,
                        )
                ...
                if state.locked_text:
                    draw_track_if_visible(frame, box, track_id, state, label_renderer, args, float(vehicle_conf), predicted=is_predicted)
                    continue
                if is_predicted and not allow_predicted_plate(state, frame_idx, args):
                    draw_track_if_visible(frame, box, track_id, state, label_renderer, args, float(vehicle_conf), predicted=True)
                    continue
                if (
                    ...
                    if is_predicted:
                        ...
                        draw_track_if_visible(frame, box, track_id, state, label_renderer, args, float(vehicle_conf))
                ...
                if ready:
                    ...
                    if args.hyperlpr_pre_ocr and args.ocr_engine == "hyperlpr3" and vehicle_crop.size:
                        ...
                        if text and ocr_conf >= args.min_ocr_conf:
                            ...
                            locked_by_pre_ocr = vote(state, text, ocr_conf, frame_idx, pseudo_candidate, args, raw_frame.shape, source="vehicle", predicted=is_predicted, run_stats=run_stats)
                            ocr_vote_recorded_this_frame = True
                            if locked_by_pre_ocr:
                                run_stats["ocr_pre_locks"] += 1
                                if emit_lock_event(recent_lock_texts, state.locked_text, frame_idx, args, run_stats):
                                    ...
                                    save_lock_evidence(output_dir, track_id, state, pseudo_candidate, run_stats)
                                else:
                                    print(f"[LOCK DEDUP] track={track_id} plate={state.locked_text} frame={frame_idx}", flush=True)
                                draw_track_if_visible(frame, box, track_id, state, label_renderer, args, float(vehicle_conf), predicted=is_predicted)
                                continue
                    if roi.size:
                        plate_rgb = cv2.cvtColor(cv2.resize(roi, (args.plate_imgsz, args.plate_imgsz)), cv2.COLOR_BGR2RGB)
                        infer_started = time.perf_counter()
                        plate_output = plate_model.infer_rgb(plate_rgb)
                        run_stats["time:plate_rknn_seconds"] += time.perf_counter() - infer_started
                        for corners, obb_score in decode_obbs(plate_output, roi, (rx1, ry1), args.plate_imgsz, args.plate_conf):
                            ...
                            plate = warp_plate(raw_frame, corners)
                            score, subscores, metrics = quality_score(plate, plate_area, obb_score)
                            candidate = Candidate(frame_idx, track_id, score, subscores, metrics, obb_score, plate_area, corners.tolist(), box, float(vehicle_conf), artifact_frame, vehicle_crop, plate)
                            text, ocr_conf = "", 0.0
                            if args.ocr_engine == "none":
                                ...
                                if not text:
                                    text, ocr_conf = recognize_hyperlpr3(hyperlpr3, plate)
                                run_stats["time:hyperlpr3_seconds"] += time.perf_counter() - ocr_started
                            else:
                            ...
                            if args.lock_on_plate_detect or args.ocr_engine == "none":
                                ...
                                cv2.polylines(frame, [corners.astype(np.int32)], True, (0, 255, 255), 2)
                                break
                            if text and not ocr_vote_recorded_this_frame and vote(state, text, ocr_conf, frame_idx, candidate, args, raw_frame.shape, source="plate", predicted=is_predicted, run_stats=run_stats):
                                if emit_lock_event(recent_lock_texts, state.locked_text, frame_idx, args, run_stats):
                                    print(f"[LOCK] track={track_id} plate={state.locked_text} frame={frame_idx} events={state.locked_consensus['event_count']}", flush=True)
                                    save_lock_evidence(output_dir, track_id, state, candidate, run_stats)
                                else:
                                    print(f"[LOCK DEDUP] track={track_id} plate={state.locked_text} frame={frame_idx}", flush=True)
                            cv2.polylines(frame, [corners.astype(np.int32)], True, (0, 255, 255), 2)
                            break
            ...
            if (
                args.video == "mipi"
                and (args.mipi_plate_scan_fallback or args.mipi_plate_scan_only)
                and plate_hits == 0
                and processed_frames % args.mipi_plate_scan_interval == 0
            ):
                plate_hits += mipi_plate_scan_fallback(
                    raw_frame,
                    frame,
                    plate_model,
                    hyperlpr3,
            ...
            for track_id, state in states.items():
                if track_id not in seen_ids and state.locked_text:
                    ...
                    if predicted:
                        draw_track_if_visible(frame, predicted, track_id, state, label_renderer, args, state.last_vehicle_conf, predicted=True)
            prune_expired_states(states, frame_idx, args, run_stats)
            elapsed = max(1e-6, time.perf_counter() - started)
            total_plate_hits = sum(state.plate_hits for state in states.values())
            cv2.putText(
                frame,
                f"frame:{frame_idx} veh:{vehicle_box_count} new_hits:{plate_hits} total_hits:{total_plate_hits} fps:{processed_frames / elapsed:.2f}",
                (20, 36),
            ...
            if args.publish_frame is not None and processed_frames % max(1, args.publish_interval) == 0:
                if preview_publisher is not None:
                    ...
                    preview_publisher.submit(frame)
                    run_stats["time:preview_submit_seconds"] += time.perf_counter() - submit_started
                else:
                    ...
                    preview = frame
                    if args.preview_max_width and frame.shape[1] > args.preview_max_width:
                        preview_height = max(1, int(round(frame.shape[0] * args.preview_max_width / frame.shape[1])))
                        preview = cv2.resize(frame, (args.preview_max_width, preview_height), interpolation=cv2.INTER_AREA)
                    cv2.imwrite(str(temporary), preview, [cv2.IMWRITE_JPEG_QUALITY, args.preview_jpeg_quality])
                    temporary.replace(args.publish_frame)
                    run_stats["time:preview_publish_seconds"] += time.perf_counter() - publish_started
            if processed_frames % args.progress_interval == 0:
                print(
                    f"Processed {processed_frames} frames (video frame {frame_idx}) | "
                    f"vehicle_boxes={vehicle_box_count} | tracks={len(states)} | new_hits={plate_hits} | total_hits={total_plate_hits} | "
        ...
        if preview_publisher is not None:
            ...
            run_stats["preview_frames_submitted"] = preview_publisher.submitted
            run_stats["preview_frames_published"] = preview_publisher.published
            run_stats["preview_frames_dropped"] = preview_publisher.dropped
            run_stats["preview_publish_errors"] = preview_publisher.errors
            run_stats["time:preview_publish_seconds"] = preview_publisher.elapsed_seconds
            if preview_publisher.last_error:
                run_stats["preview_publish_last_error"] = preview_publisher.last_error
        cap.release()
        ...
        for name in ("vehicle_rknn", "plate_rknn", "hyperlpr3", "preview_submit", "preview_publish"):
            seconds = float(run_stats.get(f"time:{name}_seconds", 0.0))
            run_stats[f"time:{name}_seconds"] = round(seconds, 3)
            run_stats[f"time:{name}_ms_per_frame"] = round(seconds * 1000.0 / max(1, processed_frames), 3)
