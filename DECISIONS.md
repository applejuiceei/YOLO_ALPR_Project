...
## 决定 34：RK3588 结构化车辆系统采用接口稳定、后端分离的 C++ 主线

决定：

- 新系统放在独立目录 `rk3588_vehicle_system`，不修改既有 Windows Python 基准 `alpr_topk_capture.py`。
- `CameraManager` 只负责采集和帧队列；检测统一依赖 `IVehicleDetector`，PC 阶段用 ONNX Runtime CPU，板端新增 RKNN 实现而不是改业务层。
- 严格按单路 detector、Cropper、颜色、ByteTrack、车牌、融合、四路的顺序推进；不能因 PC 模拟已运行就宣称 RK3588 NPU 或四路完成。
- 视频处理慢于输入时使用有界最新帧队列丢弃过期帧，并明确区分处理 FPS、源 FPS和丢帧数。

原因：

- 该结构允许后续单独替换 `.rknn` 模型并为 V4L2/RGA/zero-copy 预留边界。
- 当前 610 帧 PC 实测只有 15.423 FPS，且 `npu_used=false`；它只能证明模块接口与 CPU 功能链路，不能外推板端性能。

## 决定 35：VehicleCropper 默认返回浅 ROI，复制由边界配置显式控制

决定：

- Cropper 只处理几何裁剪，不包含检测、增强、颜色或车牌逻辑。
- 对外返回 `std::optional<VehicleCrop>`；空帧、无效框和裁剪后小于最小尺寸时明确返回无结果。
- 默认 `clone_output=0`，用 OpenCV 引用计数共享原帧像素；只有异步边界明确需要独立缓冲时才在配置中启用深拷贝。
- 扩张比例、最小尺寸和 ROI 保存上限全部从 `config.yaml` 读取。

原因：

- 610 帧实测 Cropper 每帧平均仅0.001 ms，说明浅 ROI 满足当前低拷贝目标。
- 把增强、颜色和异步复制留在独立模块或调度边界，后续才能在 RK3588 上替换为 RGA/zero-copy 而不改检测器。

## 决定 36：车辆颜色采用官方 PP-Vehicle 协议并保持模型语义透明

决定：

- 颜色分类统一依赖 `IVehicleColorClassifier`；PC 用 PP-Vehicle/PP-LCNet ONNX Runtime 实现，后续 RKNN 实现不得改变业务接口。
- 严格采用官方预处理：BGR ROI 转 RGB、缩放到 `256×192`、除以255并使用 ImageNet mean/std，ONNX 的 `N×19` 输出已含 Sigmoid，不再重复激活。
- 第一版只读取前10个颜色概率；官方 `orange/golden` 映射为系统 `other`。模型不存在 `silver` 类，因此不使用规则或占位值伪造 silver。
- `ColorResult` 同时保留当前 `color/confidence` 和未来 `primary/secondary` 字段，但本阶段不凭单色模型生成辅色。

原因：

- Paddle 与 ONNX Runtime 同输入最大绝对误差仅 `8.34e-7`，证明当前转换和预处理协议一致。
- 独立黑/白 ROI 和主视频画面均得到合理结果，但没有人工颜色真值集，不能将协议验证冒充整体准确率。
- 透明保留模型原始类别与规范映射，便于后续单独替换自训练 RKNN 颜色模型，而不让业务层依赖某个模型的标签顺序。

## 决定 37：ByteTrack 使用两阶段关联且每路独立维护状态

决定：

- Tracker 统一依赖 `IVehicleTracker`，输出 `TrackedObject`；当前实现采用 Kalman预测、Hungarian分配、高低分两阶段ByteTrack关联。
- Detector最低阈值降为0.10以保留低分框，但只有达到0.50的新框才能创建候选轨迹；候选必须再次匹配后才对外发布。
- 每个Camera创建独立ByteTracker实例，ID从各自空间维护；AI detector和颜色模型仍由后续统一调度器共享。
- 只输出本帧有真实检测匹配的已确认轨迹，不输出纯Kalman预测框，避免把估计位置当作当前识别事实。
- person、non_motor和机动车使用类别组隔离；car/bus/truck允许在机动车组内关联，以容忍检测类别短时抖动。
- 保留官方常用第一次关联代价阈值0.80。当前视频的画面底部断轨不通过继续放宽阈值掩盖，后续结合RKNN实际帧间隔和调度优化。

原因：

- 610帧中14次低分轨迹观测证明第二阶段真实生效，而非只挂ByteTrack名称。
- 每路实例隔离可防止四路ID状态相互污染，同时不要求四路复制NPU模型。
- 将断轨作为已知限制比把阈值放到0.95以上更安全；过宽同类匹配在多车场景更容易造成ID切换。

## 决定 38：VehicleFusion 区分颜色置信度、投票占比与稳定状态

决定：

- Fusion统一依赖`IVehicleFusion`，每个Camera建立独立实例并按本路`track_id`维护历史；不与Detector、Classifier或Tracker具体实现耦合。
- 第一版按单帧颜色置信度加权投票；输出颜色置信度为获胜颜色样本的平均值，`vote_ratio`为获胜权重占总有效权重的比例，两者不得混为一个字段。
- 默认至少3个有效样本且`vote_ratio>=0.60`才标记稳定；置信度低于0.50或颜色为`other`的样本不参与主色投票。
- 历史最多保留30个有效样本，并按源`frame_id`清理超过60帧未更新的Track。
- 不把投票第二名解释为真实`secondary_color`，也不在没有重识别证据时跨Track ID融合。

原因：

- 单帧属性可能波动，按Track累计能降低偶发分类错误，但只有显式稳定门槛才能让业务层区分候选和已稳定结果。
- 平均模型置信度反映模型对获胜类别的强度，投票占比反映多帧一致性，二者语义不同。
- 610帧实测Fusion更新mean/P95仅0.005/0.008ms；当前主要限制仍是CPU模型推理和Tracker断轨。

## 决定 39：模块化 C++ 车辆系统按源码子树加入 GitHub 白名单

决定：

- 将 `rk3588_vehicle_system` 的 CMake入口、一方头文件/实现、单元测试、配置、转换/运行脚本、阶段报告和目录级 `.gitignore` 纳入发布白名单。
- 不使用 `rk3588_vehicle_system/**` 全目录规则；继续排除 `build`、`runs`、`third_party`、模型二进制、视频和编译产物。
- 发布 worktree 已有未提交文件时，仅当其属于当前白名单，且目标内容与源一致或源只做追加时允许续传；删除、重命名、冲突和非白名单改动全部拒绝。
