...
def update_realtime_fps(timestamps: deque[float], now: float) -> float | None:
...
def draw_realtime_hud(
    frame: np.ndarray,
    realtime_fps: float | None,
    source_fps: float,
    frame_idx: int,
    plate_hits: int,
    metric_label: str = "plate_hits",
) -> None:
    fps_value = "--.-" if realtime_fps is None else f"{realtime_fps:.1f}"
    lines = (
        (f"FPS {fps_value} / SRC {source_fps:.1f}", (0, 255, 0)),
        (f"frame:{frame_idx} {metric_label}:{plate_hits}", (0, 0, 255)),
    )
    for line_index, (text, color) in enumerate(lines):
        origin = (20, 40 + line_index * 38)
        cv2.putText(
            frame,
            text,
            origin,
...
def build_ocr_engine(engine: str) -> tuple[Any, Any, Any, bool]:
    ...
    if engine == "hyperlpr3-rec":
        from hyperlpr3_ocr import HyperLPR3RecognitionOCR
        from plate_rec_ocr import PlateRecONNX

        ocr = HyperLPR3RecognitionOCR()
        return ocr, ocr.recognize, PlateRecONNX.is_plate_like, use_vehicle_first
...
def recognize_with_optional_deblur(
    ...
    *,
    ...
    text, confidence, original_event = perform_ocr_call(
        ocr=ocr,
        recognize=recognize,
        image=plate_crop,
        frame_idx=frame_idx,
    ...
    if original_event["error"] or deblur_model is None or not should_try_deblur(text, confidence, args):
    ...
    deblur_text, deblur_confidence, deblur_event = perform_ocr_call(
        ocr=ocr,
        recognize=recognize,
        image=restored,
        frame_idx=frame_idx,
    ...
    result["deblur_event"] = deblur_event
    result["events"].append(deblur_event)
...
def ocr_timing_summary(events: list[dict[str, Any]]) -> dict[str, float | int | None]:
    values = [
        float(event["ocr_ms"])
        for event in events
        if event.get("error") is None
        and event.get("ocr_ms") is not None
        and math.isfinite(float(event["ocr_ms"]))
    ]
    if not values:
        return {
            "count": 0,
    ...
    return {
        ...
        "p50_ms": round(percentile(values, 50.0) or 0.0, 3),
        "p95_ms": round(percentile(values, 95.0) or 0.0, 3),
        "min_ms": round(min(values), 3),
        "max_ms": round(max(values), 3),
    }


def get_last_ocr_ms(ocr: Any, fallback_started_ns: int) -> float:
    measured = getattr(ocr, "last_ocr_ms", None)
    if measured is not None and math.isfinite(float(measured)):
        return float(measured)
    return (time.perf_counter_ns() - fallback_started_ns) / 1_000_000.0
...
def make_ocr_event(
    *,
    ...
    plate_like = bool(text) and (is_plate_like is None or bool(is_plate_like(text)))
    confidence_ok = confidence is None or float(confidence) >= min_ocr_conf
    if error:
        reason = "ocr_error"
    elif not text:
        reason = "empty_text"
    ...
    return {
        ...
        "timing_scope": (
            "recognition_preprocess_inference_ctc_decode"
            if engine == "hyperlpr3-rec"
            else "recognize_call"
        ),
        "plate_like": plate_like,
        "format_valid": plate_like,
        "confidence_threshold": float(min_ocr_conf),
        "confidence_ok": confidence_ok,
        "confidence_pass": confidence_ok,
        "length_pass": bool(text) and len(text) >= 7,
...
def perform_ocr_call(
    *,
    ...
    ocr_ms = get_last_ocr_ms(ocr, started_ns)
    event = make_ocr_event(
        frame_idx=frame_idx,
        track_id=track_id,
        engine=engine,
        source=source,
...
def apply_event_vote(
    ...
    *,
    ...
    if event.get("vote_reason") is not None:
        return False
    text = event.get("text")
    if not text or len(text) < args.min_lock_text_len:
        event["length_pass"] = False
        event["vote_reason"] = "text_too_short"
        return False
    event["length_pass"] = True
    if state.locked_text:
        event["vote_reason"] = "track_already_locked"
        return False
    ...
    event["vote_attempted"] = True
    event["accepted_for_vote"] = True
    event["entered_vote"] = True
    event["locked_now"] = bool(locked_now)
    return locked_now


def record_ocr_event(
    ...
    *,
    ...
    events.append(event)
    if show_timing:
        print_ocr_event(event)


def print_ocr_event(event: dict[str, Any]) -> None:
    confidence = event.get("confidence")
    confidence_text = f"{float(confidence):.3f}" if confidence is not None else "None"
    text = event.get("text") or "<empty>"
    vote_status = "entered" if event.get("entered_vote") else f"rejected:{event.get('vote_reason')}"
    error_suffix = f" error={event['error']}" if event.get("error") else ""
    print(
        f"[OCR_SPEED] frame={event['frame_idx']} track={event['track_id']} "
        f"source={event['source']} text={text} confidence={confidence_text} "
        f"ocr_ms={float(event['ocr_ms']):.3f} format_valid={event['format_valid']} "
        f"confidence_pass={event['confidence_pass']} vote={vote_status}{error_suffix}"
    )
...
def run_ocr_for_saved_candidates(
        ...
        for candidate in state.candidates:
            try:
                ...
                for image_index, image in enumerate(images):
                    if use_vehicle_first and image_index == 0:
                        source = "topk_vehicle"
                    else:
                        source = "topk_plate"
                    text, confidence, event = perform_ocr_call(
                        ocr=_ocr,
                        recognize=recognize,
                        image=image,
                    ...
                    event["processing_mode"] = "offline_topk"
                    if event["vote_reason"] is None:
                        event["vote_reason"] = "offline_topk"
                    record_ocr_event(
                        ocr_events,
                        event,
                        show_timing=args.show_ocr_timing,
                    )
                    if event["error"]:
...
def save_outputs(
    ...
    recognition_results_path = run_dir / "recognition_results.jsonl"
    with recognition_results_path.open("w", encoding="utf-8") as f:
        for event in ocr_events:
            f.write(json.dumps(event, ensure_ascii=False) + "\n")

    timing = ocr_timing_summary(ocr_events)
    completed_calls = sum(1 for event in ocr_events if not event.get("error"))
    error_calls = len(ocr_events) - completed_calls
    run_summary = {
        ...
        "tracks": tracks_index,
        "ocr_calls": len(ocr_events),
        "average_ocr_ms": timing["mean_ms"],
        "ocr_timing_ms": timing,
        "ocr_event_counts": {
            "completed": completed_calls,
            "with_text": sum(1 for event in ocr_events if event.get("text")),
            "plate_like": sum(1 for event in ocr_events if event.get("plate_like")),
            "accepted_for_vote": sum(1 for event in ocr_events if event.get("accepted_for_vote")),
            "errors": error_calls,
        },
        "recognition_results_file": recognition_results_path.name,
        "ocr": {
            "event_file": recognition_results_path.name,
            "call_count": len(ocr_events),
            "completed_count": completed_calls,
            "error_count": error_calls,
            "format_valid_count": sum(1 for event in ocr_events if event.get("format_valid")),
            "confidence_pass_count": sum(1 for event in ocr_events if event.get("confidence_pass")),
            "entered_vote_count": sum(1 for event in ocr_events if event.get("entered_vote")),
            "latency_ms": timing,
        },
    }
...
def run_topk_capture(args: argparse.Namespace) -> Path:
    ...
    if args.live_ocr:
        ...
        try:
            live_ocr, live_recognize, live_is_plate_like, live_use_vehicle_first = build_ocr_engine(args.ocr_engine)
        except Exception as exc:
    ...
    display_timestamps: deque[float] = deque(maxlen=30)
        ...
        while True:
            ...
            if tracked_boxes:
                for track_id, box_arr, vehicle_conf in tracked_boxes:
                    ...
                    ocr_text = None
                    ocr_conf = None
                    active_ocr_event = None
                    plate_ocr_event = None
                    deblur_plate_crop = None
                    deblur_ocr_text = None
                    deblur_ocr_conf = None
                    deblur_vote_text = None
                    deblur_ocr_event = None
                    pending_plate_ocr_events: list[dict[str, Any]] = []
                    deblur_used_for_vote = False
                    ocr_vote_recorded = False
                    if live_recognize is not None and live_use_vehicle_first:
                        ocr_text, ocr_conf, vehicle_ocr_event = perform_ocr_call(
                            ocr=live_ocr,
                            recognize=live_recognize,
                            image=vehicle_crop,
                            frame_idx=frame_idx,
                            track_id=track_id,
                            engine=args.ocr_engine,
                            source="vehicle",
                            is_plate_like=live_is_plate_like,
                            min_ocr_conf=args.min_ocr_conf,
                        )
                        if vehicle_ocr_event["error"]:
                            print(
                                f"WARNING: live vehicle OCR failed at frame {frame_idx}, "
                                f"track {track_id}: {vehicle_ocr_event['error']}"
                            )

                        if ocr_text:
                            active_ocr_event = vehicle_ocr_event
                            ...
                            if vehicle_ocr_event["accepted_for_vote"]:
                    ...
                    if should_run_plate_ocr:
                        if deblur_model is None:
                            plate_ocr_text, plate_ocr_conf, plate_ocr_event = perform_ocr_call(
                                ocr=live_ocr,
                                recognize=live_recognize,
                                image=plate_crop,
                                frame_idx=frame_idx,
                            ...
                            if plate_ocr_event["error"]:
                                print(
                                    f"WARNING: live plate OCR failed at frame {frame_idx}, "
                                    f"track {track_id}: {plate_ocr_event['error']}"
                                )
                            if needs_plate_fallback:
                                ocr_text, ocr_conf = plate_ocr_text, plate_ocr_conf
                                active_ocr_event = plate_ocr_event
                        else:
                            try:
                                deblur_stats["plate_ocr_with_deblur_checks"] += 1
                                result = recognize_with_optional_deblur(
                                    live_ocr,
                                    live_recognize,
                                    plate_crop,
                                    deblur_model,
                                ...
                                deblur_ocr_event = result["deblur_event"]
                                pending_plate_ocr_events = result["events"]
                                if needs_plate_fallback:
                                    ocr_text, ocr_conf = plate_ocr_text, plate_ocr_conf
                                    active_ocr_event = result["original_event"]
                    ...
                    if active_ocr_event is not None and not ocr_vote_recorded and ocr_text:
                        locked_now = apply_event_vote(
                            active_ocr_event,
                            state,
                            args,
                        ...
                        if active_ocr_event["accepted_for_vote"]:
                            ocr_vote_recorded = True
                        if locked_now:
                            print(lock_log_message(track_id, state, frame_idx))
                    ...
                    if (
                        deblur_vote_text
                        and deblur_ocr_event is not None
                        and not ocr_vote_recorded
                    ):
                        deblur_locked_now = apply_event_vote(
                            deblur_ocr_event,
                            state,
                            args,
                            vehicle_conf=vehicle_conf,
                        ...
                        if deblur_ocr_event["accepted_for_vote"]:
                    ...
                    if pending_plate_ocr_events:
                        for event in pending_plate_ocr_events:
                            if event["vote_reason"] is None and not event["accepted_for_vote"]:
                                if event is deblur_ocr_event:
                                    if not deblur_vote_text:
                                        event["vote_reason"] = "deblur_not_selected"
                                    elif ocr_vote_recorded:
                                        event["vote_reason"] = "primary_result_already_voted"
                                elif event is not active_ocr_event:
                                    event["vote_reason"] = "primary_result_already_available"
                            record_ocr_event(
                                ocr_events,
                                event,
                                show_timing=args.show_ocr_timing,
                            )
                    elif should_run_plate_ocr and plate_ocr_event is not None:
                        record_ocr_event(
                            ocr_events,
                            plate_ocr_event,
                            show_timing=args.show_ocr_timing,
            ...
            if display_frame is not None:
                realtime_fps = update_realtime_fps(
                    display_timestamps,
                    time.perf_counter(),
                )
                draw_realtime_hud(
    ...
    if args.with_ocr:
        run_ocr_for_saved_candidates(states, args.ocr_engine, args, ocr_events)

    video_meta["deblur_stats"] = dict(deblur_stats)
    save_outputs(states, run_dir, args, video_meta, ocr_events)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--ocr-engine",
        choices=["plate-rec", "plate-rec-ort", "plate-rec-cv2", "hyperlpr3", "hyperlpr3-rec", "paddle"],
        default="hyperlpr3",
        help="OCR engine used with --live-ocr or --with-ocr.",
    )
    parser.add_argument(
        "--live-ocr",
        action="store_true",
    ...
    parser.add_argument(
        "--show-ocr-timing",
        action="store_true",
        help="Print one [OCR_SPEED] line for every OCR call, including rejected, failed, and offline Top-K results.",
    )
