...
// ============================================================================
// 馃洝锔?0. 杞︾墝瀛楃涓叉潅璐ㄦ竻娲楀櫒 (鎷︽埅妯″瀷骞昏)
// ============================================================================
...
bool initRknnModel(const char* model_path, const char* model_name, rknn_context& ctx, rknn_input_output_num& io_num) {
    std::ifstream file(model_path, std::ios::binary);
    if (!file.good()) {
        std::cerr << "ERROR: cannot open " << model_name << " model file: " << model_path << std::endl;
        return false;
    }
    file.seekg(0, std::ios::end);
    std::streamoff file_size = file.tellg();
    if (file_size <= 0) {
        std::cerr << "ERROR: empty " << model_name << " model file: " << model_path << std::endl;
        return false;
    }

    int ret = rknn_init(&ctx, (void*)model_path, 0, 0, NULL);
    if (ret != RKNN_SUCC) {
        std::cerr << "ERROR: rknn_init failed for " << model_name
                  << ": " << model_path << ", ret=" << ret << std::endl;
        return false;
    }
    ...
    if (ret != RKNN_SUCC || io_num.n_input <= 0 || io_num.n_output <= 0) {
        std::cerr << "ERROR: rknn_query IN_OUT_NUM failed for " << model_name
                  << ", ret=" << ret
    ...
    std::cout << "--> Loaded " << model_name << ": " << model_path
              << " (" << file_size << " bytes, inputs=" << io_num.n_input
              << ", outputs=" << io_num.n_output << ")" << std::endl;
    return true;
}

// ============================================================================
// 馃摎 1. 瀛楀吀璇诲彇涓?OCR CTC 瑙ｇ爜寮曟搸
// ============================================================================
std::vector<std::string> load_dict(const std::string& path) {
    std::vector<std::string> dict;
    std::ifstream file(path);
    std::string line;
    dict.push_back("blank"); // 榛樿绱㈠紩0涓?CTC blank
    while (std::getline(file, line)) {
        if (!line.empty() && line.back() == '\r') line.pop_back();
        dict.push_back(line);
    }
    dict.push_back(" "); // 鍔犱笂绌烘牸瀛楃
    return dict;
}
...
// ============================================================================
// 馃寑 3. 浜岄樁娈典笌鐘舵€佹満锛歄BB 瑙ｆ瀽銆侀€忚鎷夊钩銆佹粦鍔ㄦ姇绁ㄣ€佹姄鎷嶅厹搴?// ============================================================================
...
bool processPlateOBB(const cv::Mat& src_roi, const float* net_output_ptr, int plate_model_width, int plate_model_height,
    ...
    best_score_out = best_plate.score;

    // 缂╂斁杩樺師 (淇缁挎涔辫窇鐨勬牳蹇冮€昏緫)
    ...
    std::vector<cv::Point2f> expanded_vertices = expand_rect(ordered_vertices, 5.0f, 5.0f);
    pts_local = expanded_vertices;

    cv::Point2f dst_vertices[4] = { {0,0}, {319,0}, {319,95}, {0,95} };
    cv::Mat M = cv::getPerspectiveTransform(expanded_vertices.data(), dst_vertices);
    cv::warpPerspective(src_roi, warped_plate, M, cv::Size(320, 96)); // 鎷夊钩涓?320x96
    return true;
}
...
// ============================================================================
// 馃殌 涓荤▼搴忓紑濮?(缁堟瀬涓夋搸鍏ㄥ紑宸ヤ笟娴佹按绾?+ 鏅鸿兘鎽勫儚澶存寕杞?
// ============================================================================
int main(int argc, char** argv) {
    if (argc < 6) {
        std::cerr << "鉂?鐢ㄦ硶: ./alpr_app <瀵昏溅.rknn> <鎵剧墝OBB.rknn> <OCR璇嗗埆.rknn> <瀛楀吀.txt> <瑙嗛璺緞鎴栨憚鍍忓ごID(濡?)>" << std::endl;
        return -1;
    }
    ...
    // --- 2. 鍒濆鍖?浜岄樁娈?RKNN 寮曟搸 (鎵剧墝 OBB) ---
    rknn_context ctx_plate;
    rknn_input_output_num io_num_plate;
    if (!initRknnModel(plate_model_path, "plate_obb", ctx_plate, io_num_plate)) {
        rknn_destroy(ctx_vehicle);
        return -1;
    }
    ...
    // --- 3. 鍒濆鍖?涓夐樁娈?RKNN 寮曟搸 (OCR 璇嗗瓧) ---
    rknn_context ctx_rec;
    rknn_input_output_num io_num_rec;
    if (!initRknnModel(rec_model_path, "plate_rec", ctx_rec, io_num_rec)) {
        rknn_destroy(ctx_vehicle);
        rknn_destroy(ctx_plate);
        return -1;
    }
    ...
    const float VEHICLE_CONF_THRESHOLD = 0.35f;
    const float PLATE_CONF_THRESHOLD = 0.30f;
    const int MIN_LOCK_TEXT_LEN = 6;
    ...
    if (is_camera) {
        ...
        std::cout << "--> 馃摲 姝ｅ湪鎸傝浇鐗╃悊 USB 鎽勫儚澶?ID: " << cam_id << std::endl;
        cap.open(cam_id, cv::CAP_V4L2);
        cap.set(cv::CAP_PROP_FRAME_WIDTH, 1280);
        cap.set(cv::CAP_PROP_FRAME_HEIGHT, 720);
        cap.set(cv::CAP_PROP_FPS, 30);
    } else if (input_source == "mipi") {
        std::cout << "--> 馃摲 姝ｅ湪鎸傝浇 MIPI CSI 纭欢娴?.." << std::endl;
        ...
        std::cout << "--> 馃幀 姝ｅ湪鍔犺浇娴嬭瘯瑙嗛娴? " << input_source << std::endl;
        cap.open(input_source);
    }

    if (!cap.isOpened()) {
        std::cerr << "鉂?鑷村懡閿欒锛氭棤娉曟墦寮€瑙嗛娴佹垨鎽勫儚澶达紒" << std::endl;
        return -1;
    }

    cv::Mat orig_img;
    int frame_count = 0;
    std::cout << "--> 馃殌 缁堟瀬涓夋搸鍚姩锛屽紑濮嬫墽琛岃瘑鍒姄鎷嶏紒" << std::endl;
    ...
    while (cap.read(orig_img)) {
        ...
        for (int i = 0; i < num_anchors; ++i) {
            ...
            if (max_conf > VEHICLE_CONF_THRESHOLD && (best_class == 2 || best_class == 5 || best_class == 7)) {
        ...
        custom_NMS(boxes, confidences, class_ids, VEHICLE_CONF_THRESHOLD, 0.45f, indices);
        ...
        int plate_hit_count = 0;

        // [浜岄樁娈?+ 涓夐樁娈礭 OBB 鎵剧墝 -> OCR 璇嗗瓧
        for (const auto& strack : output_stracks) {
            ...
            cv::rectangle(orig_img, vehicle_rect, cv::Scalar(255, 0, 0), 2);
            cv::putText(orig_img, "ID:" + std::to_string(track_id) + " vehicle",
                        cv::Point(vx1, std::max(20, vy1 - 10)), cv::FONT_HERSHEY_SIMPLEX,
            ...
            bool found_plate = processPlateOBB(roi_crop, obb_out_ptr, plate_model_width, plate_model_height,
                                               ...
                                               PLATE_CONF_THRESHOLD, flattened_plate, pts_local, plate_score);
            ...
            if (found_plate && !flattened_plate.empty()) {
                ...
                std::vector<cv::Point2f> exp_rect = pts_global;
                ...
                if (!raw_ocr_text.empty()) {
                    std::cout << "[ID " << track_id << "] plate_score=" << plate_score
                              << " OCR=" << raw_ocr_text << std::endl;
                }
                bool is_stable = voter.vote(track_id, raw_ocr_text, final_result);

                if (is_stable && final_result.length() >= MIN_LOCK_TEXT_LEN) {
                    state.has_final_result = true;
                    state.final_text = final_result;
                    std::cout << "鉁?[ID " << track_id << "] 閿佸畾鍏辫瘑杞︾墝: " << final_result << std::endl;
                ...
                cv::putText(orig_img, "ID:" + std::to_string(track_id) + " no plate",
                            cv::Point(vx1, std::min(orig_img.rows - 10, vy2 + 24)),
        ...
        for (auto it = vehicle_states.begin(); it != vehicle_states.end(); ) {
            ...
            if (std::find(current_frame_track_ids.begin(), current_frame_track_ids.end(), t_id) == current_frame_track_ids.end()) {
                if (!state.was_snapshot && !state.last_frame.empty()) {
                    ...
                    if (!best_text.empty() && best_text.length() >= MIN_LOCK_TEXT_LEN) {
                        std::cout << "鈿狅笍 [ID " << t_id << "] 鐩爣涓㈠け锛岃Е鍙戝厹搴曟姄鎷嶏紒" << std::endl;
        ...
        if (frame_count % 30 == 0) {
            std::cout << "--> FPS: " << current_fps
                      << ", raw_vehicle=" << boxes.size()
                      << ", nms_vehicle=" << indices.size()
                      << ", tracks=" << output_stracks.size()
                      << ", plate_hits=" << plate_hit_count << std::endl;
        }
    }

    cap.release();
    rknn_destroy(ctx_vehicle); rknn_destroy(ctx_plate); rknn_destroy(ctx_rec);
    std::cout << "馃帀 瑙嗛澶勭悊瀹屾瘯锛佽祫婧愬畨鍏ㄩ噴鏀俱€? << std::endl;
    return 0;
}
