...
# SC850SL right-road live ALPR with native 4K browser publication and
# conservative recall improvements. This is intentionally separate from the
# balanced baseline so the validated baseline remains untouched.
...
OUTPUT_DIR="${OUTPUT_DIR:-/root/alpr_topk_rk3588/runs_sc850_video71_live_4k_accuracy}"
...
# Publishing a 4K JPEG every fourth processed frame keeps the browser image
# native-resolution without spending every frame on JPEG compression.
PUBLISH_INTERVAL="${PUBLISH_INTERVAL:-4}"
PREVIEW_MAX_WIDTH="${PREVIEW_MAX_WIDTH:-3840}"
PREVIEW_JPEG_QUALITY="${PREVIEW_JPEG_QUALITY:-70}"

# Keep one vehicle-model tile so high-speed coverage remains near the balanced
# profile. Recall is improved after vehicle detection, where evidence showed
# the largest losses.
ROAD_ROI=(0.28 0.08 0.98 0.98)

echo "SC850 ALPR profile=4k_accuracy tiles=1x1 preview=${PREVIEW_MAX_WIDTH}px/${PUBLISH_INTERVAL} frames/q${PREVIEW_JPEG_QUALITY}" >&2
